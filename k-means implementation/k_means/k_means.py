...
def calculate_centers(
    ...
    """
    c = np.zeros((num_centers, data.shape[1]))
    for c1 in range(0, num_centers):
        c[c1] = np.mean(data[c1 == classifications], axis=0)
    return c
...
def cluster_data(data: np.ndarray, centers: np.ndarray) -> np.ndarray:
    ...
    """
    d = np.zeros((data.shape[0], centers.shape[0]))
    for i, j in enumerate(centers):
        d[:, i] = np.sqrt(np.sum((data - j) ** 2, axis=1))
...
def lloyd_algorithm(
    ...
    """
    tc = data[np.random.choice(data.shape[0], num_centers, replace=False)]
    err = []
    while (True):
        c = cluster_data(data, tc)
        pc = tc
        tc = calculate_centers(data, c, num_centers)
        err.append(calculate_error(data, tc))
        if np.max(np.abs(tc - pc)) < epsilon-1:
            break
    return (tc, err)
